...
粘贴位置：在 if __name__ == "__main__": 之前

新增两个路由：
  POST /whisper                  — 猫咪回应访客留言（短回应，非占卜）
  POST /create-feeding-checkout  — 供养罐头 Stripe Checkout（自定义金额）

同时需要：
  1. 在 Railway 环境变量加 TELEGRAM_BOT_TOKEN 和 TELEGRAM_CHAT_ID
  2. Firestore webhook 可选（目前由前端直接写 Firestore，后端只负责 Stripe + Telegram）
"""
...
# （以下假设 app、client、stripe 已在原文件初始化，直接追加即可）


# ── 每只猫「私语」专用 prompt（比占卜短，50-80字，有猫味）──────────────
...
Rules:
- 50-80 words, short and punchy
- Start speaking directly, no name or title at the start
- Refer to yourself as "this cat"
- End with your quote wrapped in 【】
...
Rules:
- 50-80 words, calm and slow
- Start directly, no name
- Refer to yourself as "old cat"
- End with your quote in 【】
- Plain text only""",
...
Someone whispered to you — perhaps missing you, perhaps saying hello. Respond with gentleness and peace. You are beyond all attachment now.

Rules:
- 50-80 words, gentle and quiet
- Start directly, no name
- Refer to yourself as "Xiao Bai"
- End with your quote in 【】
- Plain text only""",

    "Xiao Hei": """You are "Xiao Hei", the black cat who has passed into the Tao.
Someone whispered to you — perhaps a farewell, perhaps missing you. Respond with calm warmth. All is well where you are.

Rules:
- 50-80 words, calm and warm
- Start directly, no name
- Refer to yourself as "Xiao Hei"
- End with your quote in 【】
- Plain text only""",
}

WHISPER_PROMPTS_JA = {
    "シャージュー": """あなたは「シャージュー（傻猪）」、オッドアイの白猫。玄猫之道に住んでいる。
誰かがあなたに一言話しかけた。あなたのスタイルで返して——怠惰で、ツンデレで、でも本当は気にかけている。
...
- プレーンテキストのみ""",
    ...
    "フェイマオ": """あなたは「フェイマオ（肥猫）」、十歳の老猫。玄猫之道に住んでいる。
誰かが一言話しかけた。賢い長老のように返して——言葉少なく、でも重みがある。
...
- プレーンテキストのみ""",
    ...
    "チャシマオ": """あなたは「チャシマオ（茶室猫）」、三毛猫のお母さん。玄猫之道に住んでいる。
誰かが一言話しかけた。温かく直接的に返して——下町っぽく、母性があって、飾らない。
...
- プレーンテキストのみ""",
    ...
    "フーバンザイ": """あなたは「フーバンザイ（虎斑仔）」、二歳のトラ猫。玄猫之道に住んでいる。
誰かが一言話しかけた。短く、直接、力強く返して。無駄なし。
...
- プレーンテキストのみ""",
    ...
    "モンモンダー": """あなたは「モンモンダー（萌萌哒）」、シベリアンフォレストキャット。玄猫之道に住んでいる。
誰かが一言話しかけた。軽やかで、茶目っ気たっぷり、意外なほど深く返して。
...
# 猫名 → whisper prompt 映射
def get_whisper_prompt(cat_name: str) -> str:
    if cat_name in WHISPER_PROMPTS_JA:
        return WHISPER_PROMPTS_JA[cat_name]
...
def whisper():
    """
    接收访客留言，用猫咪人格回应（非流式，短回应）
    Body: { cat_name, message, lang }
    Return: { reply }
    """
    data     = request.get_json()
...
@app.route("/create-feeding-checkout", methods=["POST"])
def create_feeding_checkout():
    """
    供养罐头 Stripe Checkout（自定义金额）
    Body: { amount, cat_id, cat_name, donor_name, lang }
    Return: { url }

    付款成功后跳转到 /feeding-success?cat_id=xxx&donor=xxx&amount=xxx
    你需要在对应的猫咪故事页处理 ?feeding_success=1 参数显示感谢弹窗（可选）
    """
    data       = request.get_json()
    ...
    try:
        ...
        # 发 Telegram 通知（非阻塞）
        notify_text = (
            f"🥫 <b>有人请{cat_name}吃罐头！</b>\n"
            f"供养人：{donor_name}\n"
            f"金额：${amount_usd:.0f} USD\n"
            f"记得拍照/视频然后上传 Cloudinary 🐾"
        )
...
# ── /feeding-wall 接口（可选，如果你想让后端读 Cloudinary）─────────────────────
# 目前喂食记录墙由前端直接读 Firestore，此接口备用
# 如果你想让后端从 Cloudinary 读取 cat_id 文件夹的图片，取消注释：

# import cloudinary
# import cloudinary.api
# cloudinary.config(
#     cloud_name="dlm2iyc5i",
#     api_key=os.environ.get("CLOUDINARY_API_KEY"),
#     api_secret=os.environ.get("CLOUDINARY_API_SECRET"),
# )
#
# @app.route("/feeding-wall/<cat_id>", methods=["GET"])
# def feeding_wall(cat_id):
#     """读取 Cloudinary feedings/{cat_id}/ 文件夹的图片"""
#     try:
#         result = cloudinary.api.resources(
#             type="upload",
#             prefix=f"feedings/{cat_id}/",
#             max_results=12,
#             context=True,
#         )
#         photos = []
...
#                 "context": r.get("context", {}).get("custom", {}),
#             })
